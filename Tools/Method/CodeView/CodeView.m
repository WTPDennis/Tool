...
#define Space 5
#define LineWidth (self.frame.size.width - _lineNum * 2 * Space)/_lineNum
#define LineHeight 2
...
- (void)addNotification {
    ...
    observer = [[NSNotificationCenter defaultCenter] addObserverForName:UITextFieldTextDidChangeNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
        NSInteger length = selfWeak.textField.text.length;
        
        //改变数组，存储需要画的字符
        //通过判断textfield的长度和数组中的长度比较，选择删除还是添加
        if (length > selfWeak.textArray.count) {
            [selfWeak.textArray addObject:[selfWeak.textField.text substringWithRange:NSMakeRange(selfWeak.textArray.count, 1)]];
        } else {
            [selfWeak.textArray removeLastObject];
        }
        
        //标记为需要重绘
        [selfWeak setNeedsDisplay];
        
        [selfWeak underLineHidden];
        
        [selfWeak addUnderLineAnimation];


        
        if (length == selfWeak.lineNum && selfWeak.EndEditBlcok) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.05 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                selfWeak.EndEditBlcok(selfWeak.textField.text);
                [selfWeak emptyAndDisplay];
            });
        }
        if (length > selfWeak.lineNum) {
            selfWeak.textField.text = [selfWeak.textField.text substringToIndex:selfWeak.lineNum];
            [selfWeak emptyAndDisplay];
...
- (void)addUnderLine {
    ...
    for (NSInteger i = 0; i < _lineNum; i ++) {
        ...
        line.hidden = _textArray.count > i;
...
//添加分割线
- (void)addSpaceLine {
    for (NSInteger i = 0; i < _lineNum - 1; i ++) {
        CAShapeLayer *line = [CAShapeLayer layer];
        line.fillColor = linecolor.CGColor;
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(self.frame.size.width/_lineNum * (i + 1), 1, .5, self.frame.size.height - 1)];
        line.path = path.CGPath;
        line.hidden = NO;
        [self.layer addSublayer:line];
    }
    ...
    switch (_codeType) {
        case CodeViewTypeCustom:
        {
            ...
            for (NSInteger i = 0; i < _textArray.count; i ++) {
                ...
                //起点
                CGFloat startX = self.frame.size.width/_lineNum * i + (self.frame.size.width/_lineNum - wordWidth)/2;
        ...
        case CodeViewTypeSecret:
        {
            ...
            for (NSInteger i = 0; i < _textArray.count; i ++) {
                //圆点
                CGFloat pointX = self.frame.size.width/_lineNum/2 * (2 * i + 1);
