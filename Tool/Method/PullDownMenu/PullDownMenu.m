...
//下拉菜单蒙版
@property (nonatomic, strong) Cover *coverView;
//下拉菜单底部View
...
- (UIView *)coverView
{
    if (_coverView == nil) {
        ...
        _coverView = [[Cover alloc] initWithFrame:CGRectMake(coverX, coverY, coverW, coverH)];
...
-(void)monitoringMethod:(NSNotification *)note{
    // 获取列
    ...
    NSString *indexStr = [NSString stringWithFormat:@"%@",allValues.lastObject];
    // 设置按钮标题
    if (![indexStr isEqualToString:@"third"]) {
        [btn setTitle:allValues.firstObject forState:UIControlStateNormal];
    }
    
    if ([_delegate respondsToSelector:@selector(PullDownMenuWithMessage:andIndex:)]) {
        [_delegate PullDownMenuWithMessage:allValues.firstObject andIndex:indexStr];
    }
}
...
- (void)willMoveToWindow:(UIWindow *)newWindow
{
    ...
    //在进入页面时注册通知，在离开页面时清除通知
    if (newWindow == NULL) {
        [self clear];
        [[NSNotificationCenter defaultCenter] removeObserver:self];
    }else{
        // 监听更新菜单标题通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(monitoringMethod:) name:UpdateMenuTitleNote object:nil];
    }
...
- (void)btnClick:(UIButton *)button
{
    ...
    if (button.selected == YES) { // 当按钮选中，弹出蒙版
        if ([_delegate respondsToSelector:@selector(pullDownMenu:didSelectedForCellWithButtonTitle:)]) {
            [_delegate pullDownMenu:self didSelectedForCellWithButtonTitle:button.titleLabel.text];
        }
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self.contentView.frame = frame;
             //NSLog(@"%@,%@",NSStringFromCGRect(frame),vc);
        } ];
    } else { // 当按钮未选中，收回蒙版
        [self dismiss];
    }
